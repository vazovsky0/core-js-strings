...
function isPalindrome(str) {
  let copyStr = str;
  copyStr = copyStr.replace(/[^a-z0-9а-яё]/gi, '').toLowerCase();
  const revStr = copyStr.split('').reverse().join('').toLowerCase();
  if (copyStr === revStr) {
    return true;
  }
  return false;
...
function findLongestWord(sentence) {
  const cleanedSentence = sentence.replace(/[^a-z0-9а-яё\s]/gi, '');
...
function invertCase(str) {
  const invertedStr = str.replace(/[a-zA-Z]/g, (char) =>
...
function extractNameFromTemplate(value) {
  return value.replace(/[^a-z0-9а-яё\s]/gi, '').replace('Hello ', '');
}
...
function unbracketTag(str) {
  return str.replace(/[^a-z0-9а-яё\s]/gi, '');
}
